...
async function askQuestion(question, default_input, optional = false, password) {
    ...
    if (optional)
        config.autoComplete = ({ length }) => length === 0 ? optHint : '';
    if (password)
        config.echoChar = true;
    let field = terminal_utils_1.term.inputField(config).promise;
    if (optional) {
        terminal_utils_1.term.grey(optHint);
        terminal_utils_1.term.left(optHint.length);
    }
